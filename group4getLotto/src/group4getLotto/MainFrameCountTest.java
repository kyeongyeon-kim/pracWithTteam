...
class SaveCount{
	...
	private int lottoBonusSave;
	public int[] getLottoMainSave() {
		return lottoMainSave;
	}
	public void setLottoMainSave(int[] lottoMainSave) {
		this.lottoMainSave = lottoMainSave;
	}
	public int getLottoBonusSave() {
		return lottoBonusSave;
	}
	public void setLottoBonusSave(int lottoBonusSave) {
		this.lottoBonusSave = lottoBonusSave;
	}
}

public class MainFrameCountTest extends JFrame implements ActionListener {
	...
	JComboBox saveBox;
	List<SaveCount> saveNumber = new ArrayList<>();
	private String[] strAs = new String[1];
	private JPanel main;
	JLabel[] comboLbl = new JLabel[7]; 

	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MainFrameCountTest frame = new MainFrameCountTest();
	...
	public MainFrameCountTest() {
		...
		// 회차 목록
		strAs[0] = "첫 회";
		saveBox = new JComboBox(strAs);
		saveBox.setBounds(50, 20, 70, 30);
		main.add(saveBox);
		
		for (int i = 0; i < 7; i++) {
			comboLbl[i] = new JLabel();
			comboLbl[i].setBounds(i * 50 + 150, 20, 30, 30);
			main.add(comboLbl[i]);
		}
...
//                         다이얼 로그 호출하기
						...
						SaveCount sct = new SaveCount();
						sct.setLottoMainSave(money.getRan().getLottoMain());
						sct.setLottoBonusSave(money.getRan().getLottoBonus());
						saveNumber.add(sct);
						strAs = new String[saveNumber.size()];
						for (int i = 0; i < saveNumber.size(); i++){
							strAs[i] = String.valueOf(i + 1);
						}
						main.remove(saveBox);
						saveBox = new JComboBox(strAs);
						saveBox.setBounds(50, 20, 70, 30);
						...
						saveBox.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e) {
								...
								for (int i = 0; i < 6; i++) {
									comboLbl[i].setText(String.valueOf(saveNumber.get(comboIndex).getLottoMainSave()[i]));
								}
								comboLbl[6].setText(String.valueOf(saveNumber.get(comboIndex).getLottoBonusSave()));
							}
						});
